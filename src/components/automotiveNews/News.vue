        ...
        <div class="news_list">
          ...
          <div class="news_box" style="height: 450px" v-for="item in newsList" :key="item.newsID">
            <div class="text">
              ...
              <router-link :to="'/automotiveNews/newsInfo/'+item.newsID"><h3 class="title">{{item.title}}</h3></router-link>
            </div>
          </div>
        </div>
        <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      /*获取新闻列表*/
      getNewsList(){
        this.$get(this.$api.module.operationNews + '?status=1&type=8&pageSize=' + this.pageSize + "&pageNo=" + this.pageNo).then(res => {
          if (res.status == 200){
